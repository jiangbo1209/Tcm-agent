...
class AgentSettings:
    ...
    llm_provider: str = _env("AGENT_LLM_PROVIDER", _env("LLM_PROVIDER", "openai"))
    llm_base_url: str = _env("AGENT_LLM_BASE_URL", _env("LLM_BASE_URL", ""))
    llm_api_key: str = _env("AGENT_LLM_API_KEY", _env("LLM_API_KEY", _env("RELAY_API_KEY", "")))
    llm_model: str = _env("AGENT_LLM_MODEL", _env("LLM_MODEL", "qwen-plus"))
    llm_auth_header: str = _env("AGENT_LLM_AUTH_HEADER", _env("LLM_AUTH_HEADER", "Authorization"))
    llm_timeout_seconds: int = _env_int("AGENT_LLM_TIMEOUT_SECONDS", _env_int("LLM_TIMEOUT_SECONDS", 120))
    llm_temperature: float = _env_float("AGENT_LLM_TEMPERATURE", _env_float("LLM_TEMPERATURE", 0.2))
    llm_max_tokens: int = _env_int("AGENT_LLM_MAX_TOKENS", _env_int("LLM_MAX_TOKENS", 4096))
    enable_llm_query_analysis: bool = _env_bool("AGENT_ENABLE_LLM_QUERY_ANALYSIS", True)
