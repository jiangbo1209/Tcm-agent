...
工程化的论文信息爬取工具，默认从 PostgreSQL 的 `core_file` 表读取待处理文件（不读取 PDF 正文），用文件名清洗后的标题去三个学术资源站搜索并入库：**e读 → NSTL → CNKI**。
...
- 默认从 PostgreSQL `core_file` 表读取 `status_metadata=false` 的 PDF 记录，用 `original_name` 作为搜索输入，`file_uuid` 作为唯一标识。
- 保留可选的本地 `dataset/` 扫描模式，但当前主流程不使用。
...
- 成功或 partial 记录写入 `paper_records`；失败写入 `failed_records`，运行结束导出 `outputs/failed_records.csv`（UTF-8-SIG）。
- 数据库输入模式下，成功结果写入 `lit_metadata`，并把对应 `core_file.status_metadata` 更新为 `true`。
- 默认低并发、随机停顿 2~5s，不绕过验证码、不破解登录。CNKI 的 Playwright 引导是用户**手动**点按钮完成的，不属于绕过。

## 环境要求

- Python 3.10+
- SQLite 自带；PostgreSQL async 通过 `asyncpg` 支持，已包含在 `requirements.txt`。
...
- `core_file`：默认模式，从 `core_file` 表读取待处理文件。
- `dataset`：可选旧模式，扫描本地 `dataset/`。
...
当前主流程不需要扫描本地 `dataset/`。只有把 `.env` 改成下面这样时，程序才会走本地文件夹：

```env
INPUT_SOURCE=dataset
DATABASE_URL=sqlite+aiosqlite:///./paper_info.db
DATASET_DIR=./dataset
```
...
dataset/
  Meta分析/
    中药治疗痰湿型多囊卵巢综合征不孕疗效的Meta分析.pdf
  指南/
    001_不明原因不孕症诊断与治疗中国专家共识.pdf
```

该模式也只读文件名推断标题，不读取 PDF 正文。

## core_file 数据库输入方式

远程 PostgreSQL 模式是当前默认工作流，需要把 `.env` 配成 PostgreSQL 连接串，并启用 `core_file` 输入：
...
本地 `dataset` 旧模式默认数据库是 `paper_info.db`。成功 / partial 在 `paper_records`，失败在 `failed_records`。

```bash
sqlite3 paper_info.db
.tables
select file_name, source_site, paper_type, journal, pub_year, crawl_status from paper_records limit 10;
select file_name, failure_reason, suggested_action from failed_records limit 10;
```

`paper_records` 字段含 `journal`、`pub_year`（CNKI 能填上，yidu/nstl 留空）。

使用 `INPUT_SOURCE=core_file` 时，成功 / partial 写入远程 PostgreSQL 的 `lit_metadata`：
...
- 顺序：`yidu → nstl → cnki`。前一站出 success 立即返回，不再继续。
