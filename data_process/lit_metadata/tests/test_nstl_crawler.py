"""Tests for NSTL crawler.

Run with reduced delays for faster testing:
    REQUEST_DELAY_MIN=0 REQUEST_DELAY_MAX=0 python -m pytest tests/test_nstl_crawler.py -x -v -s

Or as a standalone script:
    python tests/test_nstl_crawler.py
"""
...
@pytest.mark.asyncio
async def test_nstl_search_and_detail() -> None:
    """Search a known title via NSTL, then fetch details of the first result."""
    settings = _test_settings()
    crawler = NstlCrawler(settings)
...
if __name__ == "__main__":
    asyncio.run(test_nstl_search_and_detail())
