    ...
    def sync(
        ...
        *,
        ...
        if source in ("all", "literature"):
            for item in self.repository.fetch_literature(limit, only_failed=only_failed, dataset_id=self.dataset_id):
                results.append(self.sync_literature(item, dry_run=dry_run, force=force))
        if source in ("all", "case"):
            for item in self.repository.fetch_cases(limit, only_failed=only_failed, dataset_id=self.dataset_id):
