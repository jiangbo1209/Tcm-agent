...
from core.exceptions import CrawlerError, DetailPageParseError
from models.schemas import PaperMetadata, SearchResult
from services.crawlers.base import BaseCrawler
from utils.text import clean_text, split_authors, split_keywords, strip_html
...
class NstlCrawler(BaseCrawler):
    ...
    def _metadata_from_record(self, record: dict[str, Any], result: SearchResult) -> PaperMetadata:
        ...
        journal = self._extract_journal(record.get("hasSo"))
        pub_year = self._join_values(record.get("yea"))
    ...
    def _extract_journal(self, value: Any) -> str | None:
        if not value:
            return None
        ...
        if isinstance(value, dict):
            if value.get("f") == "tit":
                return clean_text(self.first_value(value.get("v")))
            return clean_text(self.first_value(value.get("tit")) or self.first_value(value.get("title")))
        if isinstance(value, list):
            for source_item in value:
                journal = self._extract_journal(source_item)
                if journal:
                    return journal
        return None
