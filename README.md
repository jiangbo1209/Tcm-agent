...
- 成功或 partial 记录写入远程 `lit_metadata`，并把对应 `core_file.status_metadata` 更新为 `true`。
- 失败记录写入远程 `failed_records`，运行结束可导出 `outputs/failed_records.csv`（UTF-8-SIG）作为本地运行报告。
...
`CORE_FILE_BATCH_SIZE` 控制每批从 `core_file` 读取多少条，避免一次性把全部待处理记录加载进内存。
`CORE_FILE_PENDING_LIMIT=0` 表示本次运行不限制总处理数量；设为正整数时只处理指定数量的待处理记录。
`SKIP_FAILED_RECORDS=true` 表示已经写入 `failed_records` 的 `file_uuid` 不会在后续批次里反复重试；需要重试时可以删除对应失败记录，或临时设为 `false`。

把 `ENABLE_CNKI=false` 即可完全停用 CNKI 链路（也不会触发 Playwright）。

## core_file 数据库输入方式

远程 PostgreSQL 是当前唯一工作流，需要把 `.env` 配成 PostgreSQL 连接串：
...
where status_metadata = false
  ...
  and not exists (
    select 1
    from public.failed_records fr
    where fr.file_uuid = core_file.file_uuid
  );
```
...
- `status_metadata`：成功写入 `lit_metadata` 后更新为 `true`；失败时保持 `false`，但默认会按 `failed_records.file_uuid` 跳过，避免同一文件反复失败和重复写失败记录。
...
select file_uuid, file_name, failure_reason, suggested_action
from public.failed_records
order by created_at desc
limit 10;
```
...
字段：`file_uuid`、`file_name`、`file_path`、`cleaned_title`、`attempted_sites`、`failure_reason`、`error_message`、`suggested_action`、`created_at`。
...
### 数据库连接失败
确认 `.env` 中的 `DATABASE_URL` 使用 `postgresql+asyncpg://`，并且密码里的 `@` 已写成 `%40`。
